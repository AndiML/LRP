"""Represents a sub-package, which contains the command line commands of the application."""
...
def get_command_descriptors() -> list[BaseCommandDescriptor]:
    ...
    Returns:
        list[BaseCommandDescriptor]: Returns a list containing the descriptors of all the commands of the application.
    """
    ...
    for module_path in glob(os.path.join(os.path.dirname(__file__), '**', 'command_descriptor.py')):
        ...
        command_descriptor_modules.append(importlib.import_module(f'lrp.commands.{sub_package_name}.command_descriptor'))
...
def get_command(command_name: str) -> type:
    """Retrieves the class that represents the command with the specified name.

    Args:
        command_name (str): The name of the command for which the class is to be retrieved.

    Returns:
        type: Returns the class that represents the command with the specified name.
    ...
    command_module = importlib.import_module(f'lrp.commands.{command_module_name}.command')
