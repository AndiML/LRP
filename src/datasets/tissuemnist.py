...
class TissueMnist(Dataset):
    ...
    def _load_datasets(self)-> None:
        ...
        self._training_data = TissueMNIST(
            root=self.path, split='train', as_rgb=True,size=128, download=True, transform=self.transform
        )
        ...
        self._validation_data = TissueMNIST(
            root=self.path, split='val', download=True, size=128, as_rgb=True, transform=self.transform
        )
        ...
        self._test_data = TissueMNIST(
            root=self.path, split='test', download=True,size=128,  as_rgb=True, transform=self.transform
        )
