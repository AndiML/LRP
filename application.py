...
from lrp.commands import get_command_descriptors, get_command
...
class Application:
    ...
    def run(self) -> None:
        ...
        # Checks if the user wants to disable the progress bar, if so, TQDM is disabled globally, by setting the TQDM_DISABLE environment variable
        # (unfortunately, setting the environment variable must happen before TQDM is first imported, as PyTorch also uses TQDM, it can only be
        # imported after the environment variable is set, otherwise the progress bar would not be disabled)
        ...
        # 2^32-1, because the NumPy requires seeds to be 32-bit unsigned integers)
    ...
    def configure_logging(
        ...
        """
        Configures the root logger using RichHandler for styled console output.

        Args:
            logging_level (Literal['all', 'debug', 'info', 'warning', 'error', 'critical']): The logging level.
            log_file_path (str | None): Path to the log file, if any.
        """
        root_logger = logging.getLogger()
        # Map our custom verbosity level to logging levels.
        root_logger = logging.getLogger()
        logging_level_map = {
            'all': logging.NOTSET,
            'debug': logging.DEBUG,
        ...
        # Use RichHandler with the custom console.
        console_handler = RichHandler(rich_tracebacks=True, markup=True)
        console_handler.setLevel(logging_level_map[logging_level])
        root_logger.addHandler(console_handler)

        # Optionally add a file handler with the SectioningFormatter.
        if log_file_path is not None:
            file_handler = logging.FileHandler(log_file_path)
            file_handler.setLevel(logging_level_map[logging_level])
            file_formatter = SectioningFormatter('%(asctime)s - %(levelname)s - %(message)s')
    ...
    def parse_command_line_arguments(self) -> argparse.Namespace:
        ...
        argument_parser = argparse.ArgumentParser(
            prog='lrp',
            description='A command line tool for experiments for LRP Experiments.',
            add_help=False
        )
        ...
        argument_parser.add_argument(
            ...
            '--version',
            action='version',
            version=f'LRP Experiments {__version__}',
        ...
        argument_parser.add_argument(
            ...
            action='store_true',
            help='''
                Disables the progress bar for all commands in the application. This is helpful if this application is used in a script, then the
                output of the progress bar can be really messy and hard to parse.
            '''
        )
